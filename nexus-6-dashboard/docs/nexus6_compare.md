...
The Nexus-6 Compare dashboard uses the ```NEXUS6_COMPARE``` view located on the Snowflake production environment.
...
```python publish_templates.py --template nexus6_compare.twbx```
...
```python publish_templates.py --template nexus6_compare.twbx --env dev --table-prefix DEV_```
