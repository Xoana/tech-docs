...
The Nexus-6 Compare dashboard uses the ```NEXUS6_COMPARE``` view located on the Snowflake BR layer.

## Updating the Template

The Nexus-6 Compare dashboard is generated using the ```nexus6_compare.twbx``` file.
...
```python publish_templates.py --template-to-publish nexus6_compare.twbx```
...
```python publish_templates.py --template-to-publish nexus6_compare.twbx --env dev --table-prefix DEV_```
